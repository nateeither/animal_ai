...
export default function NotifConfigScreen({ navigation }) {
    ...
    const { successGetUsers } = useSelector(state => ({
        successGetUsers: state.userReducer.successGetUsers,
    }), shallowEqual);
    ...
    return (
        <SafeAreaView style={{flex:1}}>
            ...
            <NotificationBottomNav loading={updateUserLoading} onPressFinish={() => handleUpdate()} />
            {
                frequencyModalShown &&
                <Modal
                    entry="bottom"
...
const NotificationBottomNav = (props) => {
    const { loading, onPressFinish } = props;
        ...
        <View style={{
            ...
            <View style={{
                ...
                <View style={{
                    ...
                    <CustomButton1 disabled={loading} loading={loading} title="Save" onPress={onPressFinish} style={{width:'100%'}}  />
                </View >
            </View>
        </View>
    )
